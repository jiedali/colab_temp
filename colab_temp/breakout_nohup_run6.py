		...
		with tf.variable_scope(name):
			...
			with tf.name_scope("conv2"):
				self.conv2 = tf.layers.conv2d(
					...
					kernel_size=[4, 4], strides=2, padding="VALID",
					kernel_initializer=tf.variance_scaling_initializer(),
...
# checkpoint path
checkpoint_path="./breakout_dqn_run6.ckpt"
...
with tf.Session(config=session_conf) as sess:
	...
	while True:
		...
		if step % save_steps == 0:
			...
			#           file.write('%s\n' % final_avg30_epi_rwd)
			with open('breakout_run6_nohup_mean_max_q.txt', 'w') as file:
				file.write('%s\n' % final_mean_max_q)
			with open('breakout_run6_nohup_each_episode_reward.txt', 'w') as file:
				file.write('%s\n' % final_each_episode_rwd)